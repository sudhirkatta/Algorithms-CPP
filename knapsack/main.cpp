...
template <typename T, class U> class KnapSack {
public:
  T KnapSackBottomUpDP(vector<pair<int, int>> &WeightValue,
                         const int &totalweight, int &optimumValue,
                         vector<pair<int, int>> &optimumItems);
  T KnapSackTopDownDP(vector<pair<int, int>> &WeightValue,
                        const int &totalweight, int &optimumValue,
                        vector<pair<int, int>> &optimumItems);
  T KnapScakReccur(vector<pair<int, int>> &WeightValue, int remainingWeight,
                     int currentItem, map<int, int> optimumValueMap);
  T GetHashKey(T a, U b);
};

template <typename T, class U>
T KnapSack<T, U>::KnapSackBottomUpDP(vector<pair<int, int>> &WeightValue,
  ...
  for (int row = 0; row < table.size(); row++) {
    for (int weight = 0; weight < totalweight + 1; weight++) {
      if (row == 0 || weight == 0) {
        ...
        table[row][weight] =
            max(WeightValue[row - 1].second +
                    table[row - 1][weight - WeightValue[row - 1].first],
                table[row - 1][weight]);
      } else {
...
template <class T, class U> T KnapSack<T, U>::GetHashKey(T a, U b) {
...
template <typename T, class U>
T KnapSack<T, U>::KnapScakReccur(vector<pair<int, int>> &WeightValue,
                                   int remainingWeight, int currentItem,
                                   map<int, int> optimumValueMap) {

  cout << remainingWeight << "(" << currentItem << ","
       << WeightValue[currentItem].first << ")" << endl;
  if ((remainingWeight <= 0) || (currentItem >= WeightValue.size())) {
    return 0;
  }

  int key = GetHashKey(remainingWeight, WeightValue.size() - currentItem);
  ...
  if (remainingWeight >= WeightValue[currentItem].first) {
    int A = WeightValue[currentItem].second +
            KnapScakReccur(WeightValue,
                           (remainingWeight - WeightValue[currentItem].first),
                           (currentItem + 1), optimumValueMap);
    cout << A << endl;
    int B = KnapScakReccur(WeightValue, remainingWeight, currentItem + 1,
                           optimumValueMap);
    cout << B << endl;
    maxvalue = max(A, B);
    /*maxvalue = max(WeightValue[currentItem].second+
                   KnapScakReccur(WeightValue, (remainingWeight-
       WeightValue[currentItem].first),(currentItem+1),optimumValueMap),
                   KnapScakReccur(WeightValue,
       remainingWeight,currentItem+1,optimumValueMap));
                           */
  } else {
    maxvalue = KnapScakReccur(WeightValue, remainingWeight, currentItem + 1,
                              optimumValueMap);
  }
...
template <typename T, class U>
T KnapSack<T, U>::KnapSackTopDownDP(vector<pair<int, int>> &WeightValue,
                                      const int &totalweight, int &optimumValue,
                                      vector<pair<int, int>> &optimumItems) {
  map<int, int> optimumValueMap;
...
int main(int argc, char *argv[]) {
  ...
  knapSack.KnapSackTopDownDP(WeightValue, totalweight, optimumValue,
                             optimumItems);
  cout << optimumValue << "  "
       << "\n";
  ...
  //    for (int i = 0; i < numberOfTests; i++)
  //    {
  //        KnapSackBottomUpDP(listOfTests[i], maxCapacity[i],
  //        optimumValue,optimumItems);
